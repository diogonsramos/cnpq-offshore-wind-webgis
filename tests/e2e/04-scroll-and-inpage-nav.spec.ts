...
test.describe('Navegação in-page — links de âncora no navbar', () => {
  ...
  test('T22 — navbar contém links principais', async ({ page }) => {
    const anchors = page.locator('.gh-nav-link')
    await expect(anchors).toHaveCount(4)
    await expect(anchors.filter({ hasText: 'Home' })).toBeVisible()
    await expect(anchors.filter({ hasText: 'Projeto & Metodologia' })).toBeVisible()
    await expect(anchors.filter({ hasText: 'WebGIS' })).toBeVisible()
    await expect(anchors.filter({ hasText: 'Dashboard' })).toBeVisible()
  })
